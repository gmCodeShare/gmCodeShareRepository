...
function linkGGB() {
  ggb1.instance.setValue("time", ggb2.instance.getValue("scrub")); // GGB name
}

ggb2.instance.registerObjectUpdateListener("scrub", linkGGB); // GGB name
...
ggb2.instance.registerClientListener((a) => {
  if (a.type == "select" && a.target == "Scrubber") {
    ...
    linkGGB();
    parseStrokes();
    ggb2.instance.setFixed("Scrubber", false, ggb2.instance.getMode() != 6); // GGB name
  ...
  for (let i = 0, L = allStrokes.length; i < L; i++) {
    ...
    if (cmdArr.length > 2) {
      ...
      let targetList = ggb1.instance.evalCommandGetLabels(
        `Sequence((x(Mover), y(Element(${intList}, i))), i, 1, Length(${intList}))`
      );
      ...
      let images = ggb1.instance.evalCommandGetLabels(
        "Sequence(Translate(redCart, Vector(Mover, Element(" +
          targetList +
          ",i))), i, 1, Length(" +
          targetList +
          "))"
