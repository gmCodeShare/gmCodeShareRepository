const { ggb1, button1, button2, button3, button4, text1, text2, text3, text4 } =
  components;
...
// some will have to move to platform side:
// ggb1.instance.registerObjectClickListener("button1", "start1");
ggb1.instance.registerObjectUpdateListener("timeBase", advance1);
...
function holdUp() {
  ggb1.instance.setAnimating("timeBase", false);
...
function start1() {
  ...
  holdUp();
  ggb1.instance.setAnimating("timeBase", true);
  ggb1.instance.startAnimation();
}

function advance1() {
  if (ggb1.instance.getValue("timeBase") == 1) {
...
function genericCycle(segment, target, mover, step, time, check) {
  ...
  ggb1.instance.evalCommand("SetActiveView(1)"); // this line can get chucked during authoring
