const {
  ...
  feedback1,
  text1,
  cc_sharewithclass_cd0dd229c29e_textbox1: shareText1,
  cc_sharewithclass_cd0dd229c29e_input1: shareInput1,
  cc_sharewithclass_cd0dd229c29e_button1: shareButton1,
  cc_sharewithclass_cd0dd229c29e_studentanswers1: shareAnswers1,
} = components;
...
let maxPicsA = 100;
let maxPicsB = 100;
ggb1.updateInnerData({ maxObjA: maxPicsA, maxObjB: maxPicsB });
...
//set weight of weight on right
ggb1.updateInnerData({ rightTotal: 1 });

//shadow fill mult objs (for when there are many of one obj, but 0 of the other)
ggb1.instance.setFilling('objALeftMult', shadowFill);
ggb1.instance.setFilling('objBLeftMult', shadowFill);
...
function clone(a) {
  ...
  //using plus and minus buttons to add and remove birds
  if (
    a[0] == 'mouseDown' &&
    ggb1.innerData['inRegionPlusIconObjARight'] &&
    picsA.length - 1 < maxPicsA
  ) {
    sudoCloneAToTar1();
    sortArrays();
    checkTilt();
  ...
  if (
    a[0] == 'mouseDown' &&
    ggb1.innerData['inRegionPlusIconObjBRight'] &&
    picsB.length - 1 < maxPicsB
  ) {
    sudoCloneBToTar1();
    sortArrays();
    checkTilt();
  }
  if (a[0] == 'mouseDown' && ggb1.innerData['inRegionMinusIconObjARight']) {
    removeLastAInTar1();
    sortArrays();
    checkTilt();
  }
  if (a[0] == 'mouseDown' && ggb1.innerData['inRegionMinusIconObjBRight']) {
    removeLastBInTar1();
    sortArrays();
    checkTilt();
  }
...
function snapToLanding() {
  if (target1Arr.length <= ggb1.innerData['switchToStackedObjsAt']) {
    ...
    for (let i = 0, L = currentAInTar1.length; i < L; i++) {
      ggb1.instance.setCoords(
        currentAInTar1[i],
        ggb1.instance.getListValue('target1LandingXVals', 1),
        ggb1.instance.getListValue('target1LandingYVals', 1)
      );
    }
    for (let i = 0, L = currentBInTar1.length; i < L; i++) {
      ggb1.instance.setCoords(
        currentBInTar1[i],
        ggb1.instance.getListValue('target1LandingXVals', 2),
        ggb1.instance.getListValue('target1LandingYVals', 2)
      );
    }
  }
  /*
...
function sortArrays() {
  ...
  ggb1.updateInnerData({ numObjAInArr1: currentAInTar1.length });
  ggb1.updateInnerData({ numObjBInArr1: currentBInTar1.length });
...
//initiates animation of the beam tilting when one side is heaver than the other
function checkTilt() {
...
//continues to snap points to targets as time runs
autorun(() => {
...
function sudoCloneAToTar1() {
  let picName = ggb1.instance.evalCommandGetLabels(
    'CopyFreeObject(' + picsA[picsA.length - 1] + ')'
  );
  picsA.push(picName);
  ggb1.updateInnerData({ picsALength: picsA.length });
  let pointName = ggb1.instance.evalCommandGetLabels(
  ...
  */
  ...
  xmlstring = serializer.serializeToString(xmldom);
  ggb1.instance.evalXML(xmlstring);
  target1Arr.push(pointsA[pointsA.length - 2]);
}

function sudoCloneBToTar1() {
  let picName = ggb1.instance.evalCommandGetLabels(
    'CopyFreeObject(' + picsB[picsB.length - 1] + ')'
  );
  picsB.push(picName);
  ggb1.updateInnerData({ picsBLength: picsB.length });
  let pointName = ggb1.instance.evalCommandGetLabels(
  ...
  */
  ...
  xmlstring = serializer.serializeToString(xmldom);
  ggb1.instance.evalXML(xmlstring);
  target1Arr.push(pointsB[pointsB.length - 2]);
}
...
//removes the last occurance of a pointA from target1Arr
function removeLastAInTar1() {
  let tempArr = target1Arr.filter(keepAPoints);
  let tempPoint = tempArr[tempArr.length - 1];
  if (target1Arr.includes(tempPoint)) {
    ggb1.instance.deleteObject(picsA[pointsA.indexOf(tempPoint)]);
    picsA.splice(pointsA.indexOf(tempPoint), 1);
    ggb1.instance.deleteObject(pointsAInTarget1[pointsA.indexOf(tempPoint)]);
    pointsAInTarget1.splice(pointsA.indexOf(tempPoint), 1);
    //ggb1.instance.deleteObject(pointsAInTarget2[pointsA.indexOf(tempPoint)]);
    //pointsAInTarget2.splice(pointsA.indexOf(tempPoint), 1);
    ggb1.instance.deleteObject(pointsAInDrawer[pointsA.indexOf(tempPoint)]);
    pointsAInDrawer.splice(pointsA.indexOf(tempPoint), 1);
    ggb1.instance.deleteObject(pointsA[pointsA.indexOf(tempPoint)]);
    pointsA.splice(pointsA.indexOf(tempPoint), 1);
    ggb1.updateInnerData({ picsALength: picsA.length });
    target1Arr.splice(target1Arr.indexOf(tempPoint), 1);
  }
}

//removes the last occurance of a pointB from target1Arr
function removeLastBInTar1() {
  let tempArr = target1Arr.filter(keepBPoints);
  let tempPoint = tempArr[tempArr.length - 1];
  if (target1Arr.includes(tempPoint)) {
    ggb1.instance.deleteObject(picsB[pointsB.indexOf(tempPoint)]);
    picsB.splice(pointsB.indexOf(tempPoint), 1);
    ggb1.instance.deleteObject(pointsBInTarget1[pointsB.indexOf(tempPoint)]);
    pointsBInTarget1.splice(pointsB.indexOf(tempPoint), 1);
    //ggb1.instance.deleteObject(pointsBInTarget2[pointsB.indexOf(tempPoint)]);
    //pointsBInTarget2.splice(pointsB.indexOf(tempPoint), 1);
    ggb1.instance.deleteObject(pointsBInDrawer[pointsB.indexOf(tempPoint)]);
    pointsBInDrawer.splice(pointsB.indexOf(tempPoint), 1);
    ggb1.instance.deleteObject(pointsB[pointsB.indexOf(tempPoint)]);
    pointsB.splice(pointsB.indexOf(tempPoint), 1);
    ggb1.updateInnerData({ picsBLength: picsB.length });
    target1Arr.splice(target1Arr.indexOf(tempPoint), 1);
  }
}
...
function addToTarget1() {
  if (target1Arr.length <= ggb1.innerData['switchToStackedObjsAt'] - 1) {
    target1Arr.splice(
...
/*
{"compTotals":{"geogebra":1,"textbox":2,"sharewithclass":1},"stage":"Learn","lessonInfo":"8 M5 TC L12 - Solving Historical Problems with Systems of Equations​","teacherView":false,"layout":"two col"}
*/
