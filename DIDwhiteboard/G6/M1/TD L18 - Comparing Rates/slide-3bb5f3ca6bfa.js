...
let choiceData = getFromSlide(id2, 'select1', false) || false;

let choiceNum = !choiceData.data?.selected
  ? `$\\text\\color{707070}{\[no input yet on slide ${getSlideNum(id2)}\]}$`
  : parseInt(choiceData.data.selected) + 1;

text1.updateData({
  text: `You chose song ${choiceNum} as having the faster tempo. Press Play.`,
});
