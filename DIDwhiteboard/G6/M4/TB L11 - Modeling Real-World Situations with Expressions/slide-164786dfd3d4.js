const {
  ...
  ggb1,
  feedback1,
  cc_submit_3919376f7325_textbox1,
  cc_submit_3919376f7325_input1,
  cc_submit_3919376f7325_button1,
  separator1,
...
function onInit() {
  if (!table1.data.init) {
    cc_submit_7761349fa73e_textbox1.updateData({ visible: false });
    cc_submit_7761349fa73e_input1.updateData({ visible: false });
    cc_submit_7761349fa73e_button1.updateData({ visible: false });
    cc_submit_7761349fa73e_button1.updateData({ align: 'right' });
    cc_submit_3919376f7325_input1.updateData({ text: `Let m represent ` });
    ggb1.updateData({ visible: false });
    table1.updateCol(0, { math: true, editable: false });
    table1.updateCol(1, { math: true, editable: false });

    cc_submit_3919376f7325_button1.updateData({ align: 'right' });
    table1.updateData({ init: true });
  }
}
const blankBox = '';
const textStem = `Let $m$ represent`;
autorun(() => {
  if (
    cc_submit_3919376f7325_input1.data.text == blankBox ||
    cc_submit_3919376f7325_input1.data.text == textStem
  ) {
    cc_submit_3919376f7325_button1.updateData({ disabled: true });
  } else {
    cc_submit_3919376f7325_button1.updateData({ disabled: false });
  }
});

cc_submit_3919376f7325_button1.on('click', () => {
  cc_submit_7761349fa73e_textbox1.updateData({ visible: true });
  cc_submit_7761349fa73e_input1.updateData({ visible: true });
  cc_submit_7761349fa73e_button1.updateData({ visible: true });
});
...
/*
{"compTotals":{"table":1,"geogebra":1,"textbox":1,"submit":2,"separator":1},"stage":"Learn","lessonInfo":"6 M4 TB L11 - Modeling Real-Life Situations and Expressions","teacherView":false,"layout":"two col"}
*/
