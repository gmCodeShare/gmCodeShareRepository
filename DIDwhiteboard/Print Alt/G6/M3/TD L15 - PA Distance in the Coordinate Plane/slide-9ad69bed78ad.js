const { ggb1, separator1, feedback, separator5, text3, ggb2, buttonGroup1 } =
  components;
...
function update() {
  ...
  if (ggb1.instance.getValue('c') == true && firstVert > 0) {
    text3.updateData({
  ...
  if (ggb1.instance.getValue('c') == true && firstVert <= 0) {
    text3.updateData({
      text: `$\\huge{|${secondVert}|-|${firstVert}|=${length}}$`,
    });
  }
  if (ggb1.instance.getValue('c') == false && firstHori < 0) {
    text3.updateData({
  ...
  if (ggb1.instance.getValue('c') == false && firstHori > 0) {
    text3.updateData({
      text: `$\\huge{|${secondHori}|-|${firstHori}|=${length}}$`,
    });
  }
}
