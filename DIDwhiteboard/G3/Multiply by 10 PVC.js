...
function popul8() {
	...
	switch (ggbApplet.getValue("factorCase")) {
		case 1: //3*60
			for (
				let j = 0,
					rowNumber =
						ggbApplet.getValue("factor1") *
						(ggbApplet.getValue("factor2") + 1);
				j < rowNumber;
				j++
			) {
				for (
					let i = 0, rowLength = ggbApplet.getValue("factor3");
					i < rowLength;
					i++
				) {
		...
		case 3: //(3*6)*10
			for (
				let j = 0, rowNumber = ggbApplet.getValue("factor2");
				j < rowNumber;
				j++
			) {
				for (
...
function moveIt() {
	for (let i = 0, L = allPoints.length; i < L; i++) {
		...
		ggbApplet.evalCommand(
			"Translate(" + allPoints[i] + ",time*(-sectionWidth,-sectionWidth))"
		);
	}
}
