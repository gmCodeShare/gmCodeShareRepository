...
select1.on("change", () => {
	reset();
	if (input1.data.text != "" && input1.data.text != "0") {
		boundIt(input1, 0, 9);
		popul8();
		buttonGroup1.updateData({ visible: true });
	}
	if (selectedLabels(select1) == "thousandths") {
		buttonGroup1.updateSingleButton({ disabled: false }, 1);
		buttonGroup1.updateSingleButton({ disabled: true }, 2);
	}
	if (selectedLabels(select1) == "thousands") {
		buttonGroup1.updateSingleButton({ disabled: true }, 1);
		buttonGroup1.updateSingleButton({ disabled: false }, 2);
	}
});

//multiply points by 10 on first button click until points are in thousands region
...
//divide points by 10 on second button click until points are in thousandths region
...
autorun(() => {
	...
	if (
		...
		dividing == true ||
		selectedLabels(select1) == "thousands"
	) {
		buttonGroup1.updateSingleButton({ disabled: true }, 1);
	} else {
		buttonGroup1.updateSingleButton({ disabled: false }, 1);
	...
	if (
		...
		multiplying == true ||
		selectedLabels(select1) == "thousandths"
	) {
		buttonGroup1.updateSingleButton({ disabled: true }, 2);
	} else {
		buttonGroup1.updateSingleButton({ disabled: false }, 2);
