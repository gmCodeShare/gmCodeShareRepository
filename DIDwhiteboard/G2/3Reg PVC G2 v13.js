...
function scatter() {
  ...
  for (var i = 0; i < Number; i++) {
    ...
    ggbApplet.setCoords("OnesPoint" + i, Math.random() * 9 + 27, Math.random() * 9 + 1);
...
function slideToTheLeft() {
  for (var i = 0; i < Math.floor(Number / 10) * 10; i++) {
    ...
    ggbApplet.evalCommand("VectorC" + i + "=Vector(OnesPointC" + i + ",NodeHundred(1+floor(" + i + "/100)))");
...
function takeItBackNowYall() {
  for (var i = 0; i < Math.floor(Number / 100) * 100; i++) {
...
function reset() {
  ...
  for (var i = 0; i < Number; i++) {
    ...
    ggbApplet.deleteObject("OnesPointD" + i);
    ggbApplet.deleteObject("OnesPointC" + i);
    ggbApplet.deleteObject("OnesPointB" + i);
    ggbApplet.deleteObject("OnesPoint" + i);
    ggbApplet.deleteObject("OnesStickD" + i);
    ggbApplet.deleteObject("OnesStickC" + i);
