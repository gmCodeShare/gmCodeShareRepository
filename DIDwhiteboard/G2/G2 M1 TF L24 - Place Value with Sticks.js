...
function createHundredsSticks() {
	...
	for (var i = 0; i < hundredPics; i++) {
		ggb1.instance.evalCommand(
			"HPic" +
				i +
				"=CopyFreeObject(Translate(hundredPic,Vector((SectionWidth+2,-3),(4Mod(" +
				i +
				",5)+1,SectionHeight-2-4*floor(" +
				i +
				"/5)))))"
		);
...
function createTensSticks() {
	...
	for (var i = 0; i < tenPics; i++) {
		ggb1.instance.evalCommand(
			"TPic" +
				i +
				"=CopyFreeObject(Translate(tenPic,Vector((SectionWidth+9,-3),(SectionWidth+4Mod(" +
				i +
				",5)+1,SectionHeight-2-4*floor(" +
				i +
				"/5)))))"
...
function createOnesSticks() {
	...
	for (var i = 0; i < onePics; i++) {
		ggb1.instance.evalCommand(
			"OPic" +
				i +
				"=CopyFreeObject(Translate(onePic,Vector((2*SectionWidth-5,-3),(2*SectionWidth+4Mod(" +
				i +
				",5)+1,SectionHeight-2-4*floor(" +
				i +
				"/5)))))"
...
function copy(grabbed) {
	...
	if (grabbed.type == "dragEnd") {
		...
		if (objName.includes("ten") && yEnd > -1) {
			//create a new copy of the dragged image and put the old one back in the toolbox
			ggb1.instance.evalCommand(
				"TPic" + tenPics + "=CopyFreeObject(" + objName + ")"
			);
		...
		if (objName.includes("one") && yEnd > -1) {
			//create a new copy of the dragged image and put the old one back in the toolbox
			ggb1.instance.evalCommand(
				"OPic" + onePics + "=CopyFreeObject(" + objName + ")"
			);
		...
		if (onePics == 10) {
			...
			ggb1.instance.evalCommand(
				"OTPic" +
					...
					"=Translate(OPic" +
					i +
					",AnimT(time)*Element(OnesVectors," +
					(i + 1) +
					"))"
			);
		...
		if (tenPics == 10 && onePics != 10) {
			...
			ggb1.instance.evalCommand(
				"THPic" +
					...
					"=Translate(TPic" +
					i +
					",AnimT(time)*Element(TensVectors," +
					(i + 1) +
					"))"
			);
			//at the end of the time slider, switch the sticks for a bundle and delete the translating images
