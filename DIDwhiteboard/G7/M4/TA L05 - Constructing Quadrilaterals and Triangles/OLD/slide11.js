...
ggb1.updateInnerData({ rad1: minGiven, rad2: 3, rad3: 5, rad4: 5 });
//ggb1.instance.evalCommand("SetValue(probAngle, 35 deg)"); // angles slides
button1.updateData({ visible: false });

ggb1.instance.registerAddListener(adds);

ggb1.instance.registerObjectUpdateListener("madeOne", () => {
  //autorun(() => {
  //let progress = ggb1.innerData["madeOne"];
  let polys = ggb1.instance.getAllObjectNames("quadrilateral");
  let quadCommand;
  if (polys.length != 0) {
    quadCommand = ggb1.instance.getCommandString(polys[polys.length - 1]);
  }
  if (quadCommand) {
    let choppedPoints = quadCommand
      .replace("Polygon(", "")
      .replace(")", "")
      .replaceAll(" ", "")
      .split(",");
    let pointDefs = [];
    for (let i = 0, L = choppedPoints.length; i < L; i++) {
      pointDefs.push(
        /*`(${ggb1.innerData[choppedPoints[i]][0]},${
            ggb1.innerData[choppedPoints[i]][1]
          })`*/
        `(${ggb1.instance.getXcoord(
          choppedPoints[i]
        )},${ggb1.instance.getYcoord(choppedPoints[i])})`
      );
    }
    ggb1.updateData({ storedPoints: [...pointDefs] });
  }
  //console.log(ggb1.data.storedPoints);
});
...
autorun(() => {
  if (controls.current == now + 1 && ggb1.data.sendPoints) {
    utils.RTS.sendData("slide-179f3f38deed", {
      //ggb1.updateData({
