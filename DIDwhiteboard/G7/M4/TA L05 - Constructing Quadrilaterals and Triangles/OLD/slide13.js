...
function tagFlips() {
  ...
  for (let i = 0, L = flipPolys.length; i < L; i++) {
    // let polyString = ggb1.instance.getCommandString(flipPolys[i]);
    ...
    for (let j = 0, L = vertices.length; j < L; j++) {
      ...
      // let reflectingPoint = ggb1.instance.evalCommandGetLabels(
      //   `Point(Segment((${x},${y}),(${-1 * x},${y})), time3)`
      // );
      let reflectingPoint;
      if (x == "0") {
        reflectingPoint = ggb1.instance.evalCommandGetLabels(`(${x},${y})`);
      } else {
        // let orig = ggb1.instance.evalCommandGetLabels(`(${x},${y})`);
    ...
    // ggb1.instance.setVisible(flipPolys[i], false);
...
utils.RTS.on("datachange", "slide-45d08cfa98f8", (collection) => {
  if (!collection || !collection.length) {
    return;
  }
  let newCollection = discardOldResponses(collection).reverse();
  // fakeCollection.forEach(({ data, info }, index) => {});
  newCollection.forEach(({ data, info }, index) => {
    ...
    let matchingPolyArr = allPolys.filter((thing) => {
      ...
      return (
        oldCaption.substring(0, info.device.length) == info.device &&
        oldCaption.includes("evenSteven")
      );
      // return oldCaption.includes(info.device) && oldCaption.length;
    });
    ...
    if (keepErGoin) {
      ...
      let rawPoints = [];
      ggb1.instance.setValue("magIndex", useIndex);
      ...
      ggb1.instance.setCaption(
        newPoly,
        `${info.device},${info.timestamp},evenSteven,${useIndex}`
      );
      ...
      //this is just to space the polygons nicely at the beginning
      // let shaker = ggb1.instance.evalCommandGetLabels(
      //   `Point(Segment(${cent}, ${sheriff}), time1)`
      // );
      //create the beast
      // let whirlPoly = ggb1.instance.evalCommandGetLabels(
      //   `Rotate(Translate(${newPoly},Vector(${shaker},${mover})), time2 ${angle}, ${mover})`
      // );
      let whirlPoly = ggb1.instance.evalCommandGetLabels(
...
utils.RTS.on("datachange", "slide-179f3f38deed", (collection) => {
  if (!collection || !collection.length) {
    return;
  }
  let newCollection = discardOldResponses(collection).reverse();
  ...
  newCollection.forEach(({ data, info }, index) => {
    ...
    if (keepErGoin) {
      ...
      ggb1.instance.setCaption(
        newPoly,
        `${info.device},${info.timestamp},oddTodd,${useIndex}`
      );
      ggb1.instance.setVisible(newPoly, false);
      newObjects.splice(newObjects.indexOf(newPoly), 1);
      let angle = ggb1.instance.getValue(
