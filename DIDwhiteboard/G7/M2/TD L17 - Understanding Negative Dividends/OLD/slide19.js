...
button1.on("click", () => {
  ...
  if (
    ...
    ggb1.instance.getValue("quotientNum") == quotient
  ) {
    //correct
    ...
    ggb1.instance.getValue("quotientSize") == quotient &&
    ((ggb1.instance.getValue("dividend") > 0 &&
      ggb1.instance.getValue("groupSizse") > 0) ||
      (ggb1.instance.getValue("dividend") < 0 &&
        ggb1.instance.getValue("groupSizse") < 0))
  ) {
    text2.updateData({ visible: true, text: `$${size2}$` });
    ggb1.instance.setVisible("pic2", true);
  } else {
    //incorrect
