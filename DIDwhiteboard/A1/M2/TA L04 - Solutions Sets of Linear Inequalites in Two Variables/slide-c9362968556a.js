...
autorun(() => {
  ...
  if (radio1.data.selected === "0") {
    text2.updateData({
      text: "There are an infinite number of ordered pairs of numbers with a difference that is less than $6$. Do you still think it is possible to plot every point? Explain.",
    });
    button1.updateData({ disabled: true });
  }
  if (radio1.data.selected === "1") {
    text2.updateData({
      text: "Describe how you could show the graph of the solution set without plotting every point.",
    });
    text3.updateData({ visible: false });
    input2.updateData({ visible: false });
    button3.updateData({ visible: false });
...
button2.on("click", () => {
  if (radio1.data.selected === "0") {
    // text2.updateData({
    //   text:
    //     "Since it is impossible to plot an infinite number of points, how could you represent the graph of the solution set?",
    // });
    button1.updateData({ disabled: false });
...
/*
{"compTotals":{"geogebra":1,"separator":3,"button":1,"textbox":2,"radiogroup":1,"submit":2},"stage":"Learn","lessonInfo":"9 M2 TA L04 - Solution Sets of Linear Inequalities in Two Variables","teacherView":false,"layout":"two col"}
*/
