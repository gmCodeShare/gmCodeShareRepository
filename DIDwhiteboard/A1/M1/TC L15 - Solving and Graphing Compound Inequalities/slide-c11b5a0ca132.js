...
const sampleVec = ggb1.instance.getAllObjectNames("vector")[0];
setLineFilling(sampleVec, 1, ggb1);
ggb1.instance.setVisible(sampleVec, true);
ggb1.instance.setLineThickness(sampleVec, 8);

function setLineFilling(objName, newFilling, ggbComp) {
  const ggb = ggbComp.instance || ggbComp;
  const xmlString = ggb.getXML(objName);
  const parser = new DOMParser();
  const xmlDOM = parser.parseFromString(xmlString, "application/xml");
  xmlDOM
    .getElementsByTagName("lineStyle")[0]
    .setAttribute("opacity", `${newFilling}`);
  const serializer = new XMLSerializer();
  const modifiedString = serializer.serializeToString(xmlDOM);
  ggb.evalXML(modifiedString);
}

// GGB section

ggb1.instance.registerClickListener((clicked) => {
  // run keyboard "lock it in" behavior
  dragEndEvent(clicked);
});

ggb1.instance.registerUpdateListener((updated) => {
  if (ggb1.instance.getObjectType(updated) !== "point") {
    return;
  }
  // delete points that are moved back into the bin
  const hasBeenOnLineCaptions = ["Midpoint", "Endpoint"];
  if (
    ggb1.instance.getValue(`IsInRegion(${updated}, bin)`) &&
    hasBeenOnLineCaptions.some((cap) =>
      ggb1.instance.getCaption(updated).includes(cap)
    )
  ) {
    dragEndEvent(updated);
  }
});
...
function checkIt() {
  ...
  let midShaded = shaders.every(
    (obj) => ggb1.instance.getObjectType(obj) == "segment"
  );
