const { ggb1, separator11, button2, feedback1, Textbox4, separator10, fib1 } =
  components;
...
function update1() {
  ...
  const resultHori = utils.math.evaluateLatex(fib1.getInput(1).text);
  if (!resultHori.error) {
    let horizontal2 = resultHori.value;
    ggb1.instance.setValue("tHori", horizontal2);
    ggb1.instance.setValue("tFakeHori", -horizontal2);
  }
  const resultVert = utils.math.evaluateLatex(fib1.getInput(2).text);
  if (!resultVert.error) {
    let vertical2 = resultVert.value;
    ggb1.instance.setValue("tVert", vertical2);
  }
  const resultScal = utils.math.evaluateLatex(fib1.getInput(0).text);
  if (!resultScal.error) {
    let scale2 = resultScal.value;
...
/*
{"compTotals":{"geogebra":1,"separator":2,"button":1,"textbox":2,"fillblank":1},"stage":"Learn","lessonInfo":"9 M3 TD L18 - Exploring Transformations of the Graphs of Functions","teacherView":false,"layout":"two col"}
*/
