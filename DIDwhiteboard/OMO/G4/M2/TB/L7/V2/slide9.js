...
buttonGroup1.on("click:1", () => {
  ...
  recordAttempt(1, {
    row1: getFIBvals(fib1),
    row2: getFIBvals(fib2),
    row3: getFIBvals(fib3),
    row4: getFIBvals(fib4),
  });
  switch (currentCount) {
    case 0:
...
function getFIBvals(FIBcomp) {
  const vals = FIBcomp.data.values.map((element) => element.text);
  return [...vals];
}
