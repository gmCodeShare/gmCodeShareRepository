...
ggb1.instance.registerClientListener((a) => {
  switch (a.type) {
    case "select":
      const chosenSeg1 = a.target;
      const chosenLength1 = getLength(a.target);
      // use bones of animation from previous slides for selection state
      const defString = ggb1.instance.getDefinitionString(chosenSeg1);
      // match def strings like Segment (-4, 4), (-3, 6)
      const regEx = /Segment *\((-*\d), *(-*\d)\), *\((-*\d), *(-*\d)\)/;
      const coords1 = defString.match(regEx).map((strVal) => Number(strVal));
      ...
      ggb1.instance.setCoords("Start", coords1[1], coords1[2]);
      ggb1.instance.setCoords("End", coords1[3], coords1[4]);
      ggb1.instance.setValue("inputLength", Number(chosenLength1));
...
utils.RTS.on("datachange", "slide-53edd5722d6d", (register) => {
  ...
  const classResponses = lastRegister.filter(({ data, info: { isSelf } }) => {
    if (isSelf) {
      ...
      // make a note of later segment choice to gray it out here
      if (data.chosenLength2) {
        doneAnswers.push(data.chosenLength2);
      }
    }
    return !isSelf;
  });
