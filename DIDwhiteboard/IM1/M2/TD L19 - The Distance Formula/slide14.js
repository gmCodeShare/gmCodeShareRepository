...
const prevWarning = getPrevGGB("slide-53edd5722d6d", "ggb1", {}).data
  .goBackString;
...
utils.RTS.on("datachange", "slide-53edd5722d6d", (register) => {
  ...
  myResponses.forEach(({ data: { chosenSeg1, chosenLength1, coords1 } }) => {
    if (!coords1) {
      return;
    }
    saveData({ chosenLength1 });
    ggb1.instance.setColor(chosenSeg1, ...chosenSegStyle.color);
    ggb1.instance.setLineThickness(chosenSeg1, chosenSegStyle.thickness);
  ...
  // no response with length 1?
  if (
    !myResponses.length ||
    !myResponses.some(({ data: { chosenLength1 } }) => !!chosenLength1)
  ) {
    text2.updateData({ text: prevWarning });
  }
});
...
button1.on("click", () => {
  ...
  if (guessObj.boundedVal !== guessObj.value) {
    ...
    const chosenLength1 = getData("chosenLength1");
